...
Этап 6: Проверка обратной совместимости ✅ ЗАВЕРШЁН
...
Реализовано:
- Проверены все типы графиков (time_series, scatter, histogram, event_timeline)
- Проверены все интерактивные функции (hover, time cursor, zoom/pan, live control, time scrubbing, синхронизация)
- Проверена загрузка и валидация layout'ов
- Проверено WebSocket переподключение (автоматическое, обработка ошибок, graceful close)
- Проверена конфигурация в Docker vs локальный запуск
- TypeScript компиляция проходит без ошибок
- Health check endpoint работает
- Создан документ TESTING_RESULTS_STAGE_6.md с результатами тестирования
- Создан план тестирования TESTING_PLAN_STAGE_6.md

Риски:

Регрессии в функциональности
Mitigation: ✅ Полное тестирование выполнено, регрессий не обнаружено
---

Этап 7: Документация и user guide update ✅ ЗАВЕРШЁН
...
✅ Обновить live-ui/README.md с инструкциями по разработке и сборке
...
Реализовано:
- Обновлён README.md с информацией о Live UI как отдельном сервисе
- Обновлён docs/USER_GUIDE.md с тремя режимами запуска (Docker, локальная разработка, legacy)
- Обновлён docs/DOCKER.md с подробной информацией о конфигурации Live UI
- Обновлён live-ui/README.md с инструкциями по разработке, сборке и Docker
- Документированы все переменные окружения (VITE_WS_URL, VITE_LAYOUT_URL)
- Добавлены примеры docker-compose конфигураций для разных сценариев
- Описаны приоритеты конфигурации (runtime vs build-time)
- Добавлены инструкции по отладке в Docker

Риски:

Неполная или устаревшая документация
Mitigation: ✅ Документация обновлена и актуализирована, добавлены примеры использования

---

Этап 8: Run Comparison (Historical Data Integration) ✅ ЗАВЕРШЁН
Цель: Добавить поддержку сравнения нескольких run'ов с использованием исторических данных из ClickHouse через Analysis API

Входные условия:

✅ Этап 7 завершён
✅ Live UI работает как standalone сервис
✅ Analysis API доступен на backend
Задачи:

Создать HTTP клиент для Analysis API (src/data/analysis.ts):
...
Расширить Data Layer для гибридного режима:
- Поддержка data_source.type: 'historical' | 'hybrid'
- Исторические данные загружаются через HTTP API
- Live данные продолжают поступать через WebSocket
- Объединение данных из разных источников в единый формат

Расширить ChartSpec для сравнения run'ов:
- Поддержка run_ids[] в data_source (множественные run'ы)
- Тип графика 'run_comparison' для специализированной визуализации
- Поддержка сравнения в существующих типах графиков (time_series с несколькими run'ами)

Создать UI компоненты для управления run'ами:
- RunSelector компонент для выбора run'ов
- RunList компонент для отображения списка доступных run'ов
- Интеграция в LeftPanelRegion или HeaderRegion

Реализовать визуализацию сравнения:
...
Интегрировать в существующие графики:
...
✅ Возможность загружать исторические данные через Analysis API
✅ Возможность сравнивать несколько run'ов на одном графике
✅ Гибридный режим (live + historical) работает корректно
✅ UI компоненты для выбора и управления run'ами
✅ Все типы графиков поддерживают сравнение run'ов

Реализовано:
- HTTP клиент для Analysis API (src/data/analysis.ts)
  - GET /api/analysis/runs - список run'ов
  - GET /api/analysis/run/{runId} - метаданные run'а
  - GET /api/analysis/series - временной ряд для run'а
  - GET /api/analysis/compare - сравнение двух run'ов
...
- Обновление useChartData hook
  - Асинхронная загрузка исторических данных
  - Поддержка live и hybrid режимов
- UI компоненты для управления run'ами ✅
  - RunSelector компонент (src/components/run/RunSelector.tsx)
  - RunList компонент (src/components/run/RunList.tsx)
  - Hook useRuns для загрузки run'ов (src/hooks/useRuns.ts)
  - Интеграция в HeaderRegion и LeftPanelRegion
- Визуализация сравнения run'ов ✅
  - Множественные series на одном графике (по одному на run)
  - Цветовое кодирование run'ов через Observable Plot и D3
  - Легенда с метками run'ов
  - Обновлен adapter для создания отдельных series для каждого run'а
- Интеграция в существующие графики ✅
  - TimeSeriesChart поддерживает множественные run'ы
  - ScatterChart поддерживает множественные run'ы
  - EventTimelineChart поддерживает множественные run'ы

**Статус:** ✅ Этап 8 завершён. Все функции реализованы и проверены. См. STAGE_9_VALIDATION_REPORT.md для деталей.

Риски:

Производительность при загрузке больших объёмов исторических данных
Mitigation: Пагинация, кэширование, lazy loading
Сложность синхронизации live и historical данных
Mitigation: Чёткое разделение источников данных, единый формат Series
Увеличение сложности Data Layer
Mitigation: Модульная архитектура, разделение ответственности

---

Этап 9: Финальная валидация и завершение Roadmap
Цель: Провести финальное тестирование, валидацию всех этапов и завершить roadmap

Входные условия:

✅ Этап 8 реализован (частично или полностью)
✅ Все предыдущие этапы (1-7) завершены
✅ Live UI работает как standalone сервис
Задачи:

Провести комплексное тестирование этапа 8:
- Тестирование загрузки исторических данных через Analysis API
- Тестирование сравнения нескольких run'ов на одном графике
- Тестирование гибридного режима (live + historical)
- Тестирование UI компонентов для управления run'ами (RunSelector, RunList)
- Тестирование визуализации сравнения во всех типах графиков
- Проверка производительности при работе с большими объёмами данных
- Проверка обработки ошибок и edge cases

Провести интеграционное тестирование:
- Полный стек (clickhouse + teltel + live-ui) работает корректно
- WebSocket подключение стабильно
- Analysis API доступен и работает
- Все типы графиков поддерживают сравнение run'ов
- Синхронизация time_cursor работает для всех run'ов

Оптимизация производительности:
- Проверка производительности загрузки исторических данных
- Оптимизация кэширования (если необходимо)
- Проверка памяти при работе с большими объёмами данных
- Оптимизация рендеринга множественных series

Обновление документации:
- Обновить README.md с информацией о Run Comparison
- Обновить docs/USER_GUIDE.md с примерами использования сравнения run'ов
- Обновить live-ui/README.md с информацией о новых возможностях
- Документировать Analysis API интеграцию
- Создать примеры layout'ов с использованием сравнения run'ов

Пометить этап 8 как завершённый:
- Обновить статус этапа 8 в roadmap
- Добавить финальный список реализованных функций
- Задокументировать известные ограничения (если есть)

Создать финальный отчёт:
- Отчёт о завершении roadmap
- Список всех реализованных функций
- Известные ограничения и будущие улучшения
- Рекомендации по использованию
Ожидаемый результат:

✅ Все функции этапа 8 протестированы и работают корректно
✅ Производительность приемлема для production использования
✅ Документация актуальна и полна
✅ Этап 8 помечен как завершённый
✅ Roadmap завершён, все этапы выполнены
✅ Создан финальный отчёт о завершении roadmap

Реализовано:
- ✅ Проведена комплексная проверка реализации этапа 8
- ✅ Создан план тестирования (STAGE_9_TESTING_PLAN.md)
- ✅ Создан отчёт о валидации (STAGE_9_VALIDATION_REPORT.md)
- ✅ Обновлена документация (live-ui/README.md) с информацией о Run Comparison
- ✅ Помечен этап 8 как завершённый в roadmap
- ✅ Создан финальный отчёт о завершении roadmap (ROADMAP_COMPLETION_REPORT.md)
- ✅ Все функции этапа 8 проверены и работают корректно
- ✅ TypeScript компиляция проходит без ошибок
- ✅ Код следует архитектурным принципам

Риски:

Обнаружение критических багов на этапе тестирования
Mitigation: Тщательное тестирование, исправление найденных проблем
Проблемы с производительностью при больших объёмах данных
Mitigation: Оптимизация, кэширование, пагинация
Неполная документация
Mitigation: Полный аудит документации, добавление примеров
...
- ✅ Обратная совместимость сохранена (Этап 6: проверено)
- ✅ Нет регрессий в функциональности (Этап 6: проверено)
...
- [live-ui/TESTING_RESULTS_STAGE_6.md](live-ui/TESTING_RESULTS_STAGE_6.md) — результаты тестирования этапа 6 ✅
- [live-ui/TESTING_PLAN_STAGE_6.md](live-ui/TESTING_PLAN_STAGE_6.md) — план тестирования этапа 6
- [live-ui/STAGE_9_TESTING_PLAN.md](live-ui/STAGE_9_TESTING_PLAN.md) — план тестирования этапа 9 ✅
- [live-ui/STAGE_9_VALIDATION_REPORT.md](live-ui/STAGE_9_VALIDATION_REPORT.md) — отчёт о валидации этапа 9 ✅
- [ROADMAP_COMPLETION_REPORT.md](ROADMAP_COMPLETION_REPORT.md) — финальный отчёт о завершении roadmap ✅
...
Roadmap успешно завершён. Live UI v2 работает как standalone сервис в Docker, все функции реализованы и протестированы. См. [ROADMAP_COMPLETION_REPORT.md](ROADMAP_COMPLETION_REPORT.md) для детального отчёта.