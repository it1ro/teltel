...
### ⏳ Этап 5: Удаление web/ из Docker образа — В ОЖИДАНИИ

**Статус:** Ожидает начала

**План действий:**
1. Удалить строку `COPY --from=builder /build/web /app/web` из `Dockerfile:28`
2. Проверить `.dockerignore`: убедиться, что `web/` не копируется
