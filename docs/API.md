...
- **Описание:** Основной интерфейс для наблюдения за телеметрией в реальном времени
...
- **Доступ:** `http://localhost:8080` (при локальном запуске) или `http://localhost:8081` (в Docker)

### Analysis UI
- **Endpoint:** `GET /analysis.html`
- **Описание:** Интерфейс для анализа завершённых run'ов
- **Доступ:** `http://localhost:8080/analysis.html` (при локальном запуске) или `http://localhost:8081/analysis.html` (в Docker)
