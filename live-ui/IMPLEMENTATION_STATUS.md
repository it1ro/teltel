...
## 🚧 Roadmap: nginx proxy и переход на относительные пути

### Этап 1: Аудит HTTP и WebSocket обращений ✅ ЗАВЕРШЁН

- [x] **Аудит всех использований абсолютных URL**
  - Найдены все использования `getWebSocketUrl()` и `getApiBaseUrl()`
  - Найдены все прямые обращения к `localhost:8080/8081`
  - Найдены все использования `fetch` с абсолютными URL
  - Найдены все создания WebSocket с абсолютными URL
  - Составлен список файлов для изменения
  - Создан документ `AUDIT_HTTP_WS_URLS.md` с результатами аудита
...
### Этап 6: План проверки dev/prod parity ✅ ЗАВЕРШЁН

- [x] **Настройка Vite dev server proxy**
  - Настроен proxy для `/api/*` → `http://localhost:8080`
  - Настроен proxy для `/ws` → `ws://localhost:8080` с WebSocket поддержкой
  - Обновлён `vite.config.ts` с proxy конфигурацией
  - Используются относительные пути в коде (одинаково для dev и prod)

- [x] **Создание плана dev/prod parity**
  - Создан документ `DEV_PROD_PARITY_PLAN.md` с описанием настроек
  - Описаны конфигурации Vite proxy и nginx proxy
  - Сравнение dev и prod окружений
  - План тестирования для обоих окружений

- [x] **Обработка edge cases**
  - Разные порты в dev и prod (решение через относительные пути)
  - Разные протоколы (http vs https в будущем)
  - Обработка ошибок подключения
  - Различия в таймаутах и заголовках

**Результаты:**
- ✅ `vite.config.ts` обновлён с proxy настройками
- ✅ Документ `DEV_PROD_PARITY_PLAN.md` создан
- ✅ План тестирования для dev и prod окружений готов
- ✅ Edge cases задокументированы и обработаны

**Следующий этап:** Документация и фиксация решения (Этап 7)
...
## 🚧 Этап 8: Run Comparison (Historical Data Integration) — В ПРОЦЕССЕ

### Этап 8.1: HTTP клиент для Analysis API ✅ ЗАВЕРШЁН

- [x] **AnalysisClient** (`src/data/analysis.ts`)
  - GET /api/analysis/runs - список run'ов с фильтрацией
...
### Этап 8.2: Расширение Data Layer для гибридного режима ✅ ЗАВЕРШЁН

- [x] **Расширение DataLayer** (`src/data/layer.ts`)
  - Поддержка data_source.type: 'historical' | 'hybrid'
  - Кэширование исторических данных
  - Метод loadHistoricalData() для загрузки через Analysis API
  - Объединение live и historical данных в единый формат
  - Асинхронный метод getSeries() для поддержки исторических данных

### Этап 8.3: Расширение ChartSpec ✅ ЗАВЕРШЁН

- [x] **Расширение типов** (`src/types/index.ts`)
...
### Этап 8.4: Обновление hooks ✅ ЗАВЕРШЁН

- [x] **useChartData hook** (`src/hooks/useChartData.ts`)
  - Асинхронная загрузка исторических данных
  - Поддержка live, historical и hybrid режимов
  - Оптимизация обновлений (только для live данных)

### Этап 8.5: UI компоненты для управления run'ами ✅ ЗАВЕРШЁН

- [x] **RunSelector компонент** (`src/components/run/RunSelector.tsx`)
  - Выбор run'ов для сравнения (одиночный и множественный)
  - Поддержка фильтров (status, sourceId, daysBack)
  - Интеграция с shared_state.selected_run
  - Визуальное отображение выбранных run'ов с метаданными
- [x] **RunList компонент** (`src/components/run/RunList.tsx`)
  - Отображение списка доступных run'ов
  - Сортировка (running первыми, затем по дате)
  - Детальная информация о run'ах (дата, длительность, кадры, события)
  - Интеграция с shared_state.selected_run
  - Кнопка обновления списка
- [x] **Интеграция в регионы**
  - RunSelector интегрирован в HeaderRegion
  - RunList интегрирован в LeftPanelRegion
- [x] **Hook useRuns** (`src/hooks/useRuns.ts`)
  - Загрузка списка run'ов через Analysis API
  - Загрузка метаданных конкретного run'а
  - Обработка ошибок и состояния загрузки

### Этап 8.6: Визуализация сравнения run'ов ✅ ЗАВЕРШЁН

- [x] Множественные series на одном графике (по одному на run)
  - Обновлен adapter для создания отдельных series для каждого run'а
  - Series ID включает run_id для идентификации
- [x] Цветовое кодирование run'ов
  - Observable Plot графики используют автоматическое цветовое кодирование
  - D3 графики используют color scale для run'ов
  - Поддержка палитры из ChartSpec
- [x] Легенда с метками run'ов
  - Observable Plot автоматически отображает легенду для множественных series
  - Легенда показывает run_id для каждого series
  - Настройка через chartSpec.legend
- [x] Синхронизация time_cursor для всех run'ов
  - Работает через существующий механизм shared_state.time_cursor

### Этап 8.7: Интеграция в существующие графики ✅ ЗАВЕРШЁН

- [x] **TimeSeriesChart поддерживает множественные run'ы**
  - Извлечение run_id из series.id
  - Цветовое кодирование через Observable Plot
  - Легенда с метками run'ов
- [x] **ScatterChart поддерживает множественные run'ы**
  - Извлечение run_id из series.id
  - Цветовое кодирование через Observable Plot
  - Легенда с метками run'ов
- [x] **EventTimelineChart поддерживает множественные run'ы**
  - Извлечение run_id из series.id
  - Цветовое кодирование по run_id через D3 color scale
  - Приоритет цветового кодирования: run_id > type/channel (если множественные run'ы)

---

## 🚫 Что НЕ реализовано (следующие этапы)

- ❌ Run Overview компонент (отображение метаданных run'а)
