...
**Примечание:** В dev-режиме Vite dev server автоматически проксирует запросы:
- `/api/*` → `http://localhost:8080/api/*`
- `/ws` → `ws://localhost:8080/ws`

Все запросы используют относительные пути, что обеспечивает единообразие с production окружением.
...
# Запуск контейнера
docker run -d -p 3000:80 teltel-live-ui:latest
...
- Все запросы идут через `http://localhost:3000` (единый origin, нет CORS)
- `/api/*` проксируется к `http://teltel:8080/api/*` (внутри Docker сети)
- `/ws` проксируется к `ws://teltel:8080/ws` (WebSocket upgrade)

**Преимущества Docker:**
- ✅ Production-ready сборка (оптимизированный Vite build)
- ✅ nginx как единая точка входа (нет CORS проблем)
- ✅ Относительные пути (единообразие dev/prod)
- ✅ Health checks и зависимости между сервисами
- ✅ Изолированная среда выполнения

Подробнее см. [NGINX_PROXY_ARCHITECTURE.md](./NGINX_PROXY_ARCHITECTURE.md)
...
2. **Stage 2 (serve):** Nginx для отдачи статических файлов и проксирования
   - Копирование собранных файлов из builder stage
   - Конфигурация nginx для SPA и proxy (API, WebSocket)
   - Проксирование `/api/*` и `/ws` к backend

**Файлы:**
- `Dockerfile` - multi-stage build конфигурация
- `nginx.conf` - конфигурация nginx для production (SPA + proxy)

## Конфигурация

Live UI использует **относительные пути** для всех подключений к backend, что обеспечивает единообразие между dev и production окружениями.

### Архитектура подключений

#### Относительные пути

Live UI использует относительные пути для всех запросов:
- **WebSocket:** `/ws` (вместо абсолютного `ws://localhost:8081/ws`)
- **HTTP API:** `/api/*` (вместо абсолютного `http://localhost:8081/api/*`)

#### Dev-режим (Vite dev server)

В dev-режиме Vite dev server автоматически проксирует запросы:
...
- Автоматическое проксирование в dev-режиме
- Нет необходимости в конфигурации через environment variables
...
**Важно:** После миграции на относительные пути конфигурация через environment variables больше не требуется. Все запросы идут через nginx proxy на относительных путях.
